(ns escuela.core
  (:gen-class)
  (:require [escuela.models.crud :refer [config db KEY Query]]
            [escuela.routes :refer [escuela-routes]]
            [escuela.admin :refer [admin-routes]]
...
(defn wrap-login [hdlr]
  (fn [req]
    (try
      (if (nil? (session/get :user_id)) (redirect "/admin") (hdlr req))
      (catch Exception _
